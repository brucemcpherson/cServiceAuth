...
var ServiceAuth = function () {
   ...
   * @return {object] the tokenpacket
   */
  self.getTokenResult = function () {
    return tokenPacket_;
  };
  /**
  ...
  self.hasToken = function () {
    ...
    return tokenPacket_ &&  
      tokenPacket_.claims.exp > Math.floor(new Date().getTime()/1000) + settings_.minimumTokenLifeSeconds &&
  ...
  function generateJWT_ () {
    ...
    // generate the jwt
    var jwt = encode_(JSON.stringify(tokenPacket_.header)) + "." 
               + encode_(JSON.stringify(tokenPacket_.claims));
  ...
  function getClaims_  (data) {
    ...
    var claims = {
      ...
      "exp":now + (duration_ || settings_.defaultDuration),
      "iat":now
    };
